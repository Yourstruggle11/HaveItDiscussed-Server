...
export const addNewQuestion = catchAsync(async (req, res, next) => {
    ...
    const data = await questionService.postNewQuestion(
        questionTitle,
        questionBody,
        postedBy,
        keywords
...
export const getAllQuestions = catchAsync(async (req, res, next) => {
    ...
    const { totalQuestions, questions } = await questionService.getAllQuestions(
        search,
        pageQuery,
        limitQuery
    )
...
export const likeDislikeToggle = catchAsync(async (req, res, next) => {
    ...
    const { id } = req.user
    const { saveLike, message, likes } = await questionService.likeSingleQuestion(
        slug,
        id
    )
    return res.json({
